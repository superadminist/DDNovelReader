...
- On Windows 10, use a transparent main Qt/WebEngine surface and a single anti-aliased 22px CSS clip for normal-state corners; never apply a binary native region that creates jagged black corner pixels. Keep maximized/fullscreen edges square, and verify native resize/restore and playback in the real desktop host after changing this composition path. Never apply a native region to the floating window.
...
- Keep Edge playback current-sentence-first: cap lookahead at 8 sentences and 2 background requests, retry one transient failure with the same voice and rate, and reuse a small bounded set of recent successful sentence audio.
